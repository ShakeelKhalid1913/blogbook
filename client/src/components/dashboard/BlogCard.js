...
function BlogCard (props) {
  const blob = new Blob([Int8Array.from(props.blogInfo.image.data.data)], {
    type: props.blogInfo.image.contentType
  })
    ...
    <>
      ...
      >
        <Skeleton avatar active loading={props.loading}>
          <List.Item.Meta
            avatar={
              <Avatar style={{ color: 'red', backgroundColor: '#fde3cf' }}>
                S
              </Avatar>
            }
          ...
          />
          <div>{props.blogInfo.content}</div>
        </Skeleton>
      </List.Item>
    </>
  )
